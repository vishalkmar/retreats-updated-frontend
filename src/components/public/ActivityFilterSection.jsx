              ...
              if (loadingActivities) {
                return (
                  <SwiperSlide key={idx} style={{ width: 'auto' }}>
                    <div className="w-32 h-24 bg-slate-100 rounded-xl animate-pulse" />
                  </SwiperSlide>
                );
              }
              ...
              return (
                <SwiperSlide key={a.id} style={{ width: 'auto' }}>
                  <button
                    ...
                    className={`w-32 sm:w-36 group rounded-xl overflow-hidden border-2 transition ${
                      active
                  ...
                  >
                    <div className="aspect-[4/3] bg-slate-100">
                      {a.imageUrl ? (
                        <img src={fileUrl(a.imageUrl)} alt={a.name} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-ink-muted bg-brand/5">
                          <ActivityIcon size={24} />
                        </div>
                      )}
                    </div>
                    <div className={`px-2 py-2 text-center text-sm font-medium ${active ? 'bg-brand text-white' : 'bg-white text-ink'}`}>
                      {a.name}
                    </div>
                  </button>
                </SwiperSlide>
